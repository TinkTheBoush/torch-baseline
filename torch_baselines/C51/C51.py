...
class C51:
    ...
    def _train_step(self, steps):
        ...
        self.model.train()
        distribution = self.model(obses).gather(1,actions)
        with torch.no_grad():
            if self.double_q:
                next_actions = (self.model(nxtobses)*self.model.mean_bar).mean(2).max(1)[1].view(-1,1)
            else:
                next_actions = (self.target_model(nxtobses)*self.target_model.mean_bar).mean(2).max(1)[1].view(-1,1)
            next_distribution = dones*self.target_model(nxtobses).gather(1,next_actions)
            next_categorial_bar = self.model.categorial_bar*self.gamma + rewards
