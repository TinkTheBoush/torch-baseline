...
class C51(Q_Network_Family):
    ...
    def _train_step(self, steps):
        ...
        vals = self.model(obses).gather(1,actions.unsqueeze(-1).repeat_interleave(self.categorial_bar_n, dim=2)).squeeze()
        with torch.no_grad():
            ...
            if self.munchausen:
                ...
                tau_log_pi_next = (sum_q - sum_q.max(1)[0].unsqueeze(-1) - self.munchausen_entropy_tau*logsum).unsqueeze(-1)
    ...
    def learn(self, total_timesteps, callback=None, log_interval=1000, tb_log_name="C51",
