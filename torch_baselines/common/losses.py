...
class WeightedMSELoss(_Loss):
    ...
    def forward(self, input: Tensor, target: Tensor, weight : Tensor) -> Tensor:
        ...
        return (F.mse_loss(input, target, reduction='none') * weight).squeeze().mean(-1)
...
class WeightedHuber(_Loss):
    ...
    def forward(self, input: Tensor, target: Tensor, weight : Tensor) -> Tensor:
        return (F.smooth_l1_loss(input, target, reduction='none', beta=self.beta)* weight).squeeze().mean(-1) 
