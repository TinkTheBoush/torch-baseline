...
class TD3(Deterministic_Policy_Gradient_Family):
    ...
    def actions(self,obs,befor_train):
        if not befor_train:
            with torch.no_grad():
                actions = np.clip(self.actor(convert_states(obs,self.device,torch.half)).cpu().numpy() + 
    ...
    def _train_step(self, steps, grad_step):
        ...
        if self.prioritized_replay:
            ...
            indexs = data[6]
            new_priorities = np.abs((targets - vals1).squeeze().cpu().clone().numpy()) + \
                            np.abs((targets - vals2).squeeze().cpu().clone().numpy()) + self.prioritized_replay_eps
